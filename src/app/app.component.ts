...
export class AppComponent implements OnInit{
  ...
  getForecastWeather() {
    return this.weatherService.getForecastWeatherByCityName(this.cityName).subscribe(
        (data) => {this.forecastInfo = data;
                   this.forecastWeather = data?.list;
                   this.forecastTimezone = this.changeForecastTimeZone(this.forecastWeather);
                   this.weatherFiveDays = this.getFiveDaysWeather(this.forecastTimezone);
                   this.isError = false;
                  },
        (error) => {
          this.error = error.message;
